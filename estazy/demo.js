...
function renderTopStats() {
  $('topStats').innerHTML = statRow([
    ...
    { label: 'Cash left over', value: usd(T.net), sub: 'after everything, including investing', tone: T.net < 0 ? 'bad' : 'good' },
  ]);
  ...
  return [
    {
      ...
      body: `A dollar in the investment pile earned 1.1 cents over the quarter. A dollar spent on ads earned ${(T.kept / T.ads - 1).toFixed(2)} of a dollar. Nothing here is losing money and you finished ${usd(T.net)} up. The only question is which of the two engines gets fed next, and one of them is about seventy times faster than the other.`,
...
function renderMoney() {
  ...
  columnChart($('flowChart'), months.map(monthName),
    ...
    (v) => usd(v));
  $('flowNote').textContent = `June and September are part months here, so their bars are short for calendar reasons, not financial ones. Investing is drawn separately because it is not spending. It is the biggest single call you make each month, and right now it goes to the slower of the two engines.`;
...
function renderBriefing() {
  $('briefing').innerHTML = `
    ...
    <p>The quarter closed <strong>${usd(T.net)}</strong> up on cash, on ${usd(T.in)} in and
      ${usd(T.out)} out, with ${usd(T.invest)} put away on top of that. Nothing here needs rescuing.
      The one thing worth changing is where the spare money goes: it is sitting in a pile earning
      ${BLENDED.toFixed(1)}% a year while the shop beside it turns ${T.perAdDollar.toFixed(2)}x in
      about a month.</p>
